...
def generate_dhcp_hosts(bindings):
    """Generate a flat ISC dhcpd.hosts from the inventory bindings.

    Only entries with a hostname, a real MAC and an IP are written.  No
    reservation carries network or provisioning options: `host` is ISC's
    innermost scope, so an option set here would beat the subnet the address
    belongs to and keep provisioning a pool whose ZTP URL is empty.
    """
    lines = [
        '# /etc/dhcp/dhcpd.hosts - Generated by LLDPq',
        '# Reservation options come from the subnet that contains each address.',
        '',
    ]
...
def dhcp_hosts_bindings_unlocked():
    """Bindings that dhcpd.hosts is regenerated from, inventory.json first.

    Installations that predate inventory.json only have the reservation file
    itself; regenerating from an empty inventory would delete every one of
    them.
    """
    if os.path.exists(INVENTORY_FILE):
        try:
            with open(INVENTORY_FILE, 'r') as handle:
                bindings = (json.load(handle) or {}).get('bindings', [])
        except (OSError, ValueError):
            bindings = []
        if bindings:
            return bindings
    legacy = [entry for entry in parse_dhcp_hosts(get_dhcp_hosts_path())
...
def _action_save_bindings_locked():
    ...
    try:
        ...
        # A reservation outside every pool is not a syntax error: dhcpd parses
        # it and then silently never serves it.  Judge the final state against
        # the pools that are live right now, exactly as DHCP Config Save judges
        # a new pool set against the inventory that is live right now.
        live_pools, live_pools_error = read_live_dhcp_pools()
        if live_pools_error:
            result_json({'success': False, 'error': live_pools_error,
                         'rollback_performed': False})
        pool_errors = validate_bindings_against_pools(bindings, live_pools)
        if pool_errors:
            result_json({'success': False, 'error': ' '.join(pool_errors),
                         'rollback_performed': False})
...
# ======================== DHCP POOLS ========================
# Pool 1 is the directly attached pool: it must contain an IPv4 address of the
# single listen interface.  Pool 2..N serve clients in other VLANs and only
# ever see a packet when an external DHCP relay on that VLAN's gateway forwards
# it here; dhcpd then picks the pool by matching giaddr against the sibling
# subnet declaration.  LLDPq neither configures nor verifies that relay.

MAX_DHCP_POOLS = 8
DHCP_POOL_NAME_MAX_LENGTH = 64
DHCP_POOL_NAME_MARKER = '#LLDPQ pool name='
DHCP_POOL_NAME_MARKER_PATTERN = re.compile(
    r'^\s*#LLDPQ\s+pool\s+name="([^"\r\n]*)"\s*$'
)
DHCP_POOL_NAME_CHARSET = re.compile(r'[A-Za-z0-9 ._-]+')
DEFAULT_DHCP_LEASE_TIME = 172800


def _mask_dhcp_comments(text):
    """Return (scan, plain) copies of a dhcpd.conf with comments blanked out.

    Both keep every original offset.  `scan` also blanks braces and semicolons
    inside quoted strings so block scanning cannot be steered by a value;
    `plain` keeps string contents intact so values can be read back verbatim.
    """
    scan = list(text)
    plain = list(text)
    quote = None
    ...
    for index, character in enumerate(text):
        ...
        if character in '"\'':
            ...
            plain[index] = ' '
            in_comment = True
    return ''.join(scan), ''.join(plain)


def _iter_dhcp_blocks(scan, start, end):
    """Yield (statement_start, header, body_start, body_end) at one brace level."""
    depth = 0
    ...
    while index < end:
        ...
        if character == '{':
            if depth == 0:
                header = scan[statement_start:index]
                header_start = statement_start
                body_start = index + 1
            depth += 1
        elif character == '}':
            if depth > 0:
                depth -= 1
                if depth == 0:
                    yield header_start, header, body_start, index
...
def _dhcp_block_keyword(header):
    fields = header.split()
    return fields[0] if fields else ''


def _read_dhcp_pool_name_marker(text, span_start, span_end):
    """Read the machine-written name marker directly above a subnet block."""
    lines = text[span_start:span_end].splitlines()
    for line in reversed(lines[:-1]):
        if not line.strip():
            continue
        match = DHCP_POOL_NAME_MARKER_PATTERN.match(line)
        return match.group(1).strip() if match else ''
    return ''


def _read_dhcp_subnet_block(plain, header, body_start, body_end):
    """Read one subnet declaration into the pool shape the UI exchanges."""
    match = re.match(
        r'^\s*subnet\s+([\d.]+)\s+netmask\s+([\d.]+)\s*$', header
    )
    if not match:
        return None
    body = plain[body_start:body_end]
    pool = {
        'name': '', 'subnet': match.group(1), 'netmask': match.group(2),
        'range_start': '', 'range_end': '', 'gateway': '', 'dns': '',
        'domain': '', 'provision_url': '',
        'lease_time': str(DEFAULT_DHCP_LEASE_TIME),
    }
    found = re.search(r'\brange\s+([\d.]+)\s+([\d.]+)\s*;', body)
    if found:
        pool['range_start'] = found.group(1)
        pool['range_end'] = found.group(2)
    found = re.search(r'\boption\s+routers\s+([\d.]+)', body)
    if found:
        pool['gateway'] = found.group(1)
    found = re.search(r'\boption\s+domain-name-servers\s+([\d.]+)', body)
    if found:
        pool['dns'] = found.group(1)
    found = re.search(r'\boption\s+domain-name\s+"([^"]*)"', body)
    if found:
        pool['domain'] = found.group(1)
    found = re.search(
        r'\boption\s+cumulus-provision-url\s+(?!code\b)"([^"]*)"', body
    )
    if found:
        pool['provision_url'] = found.group(1)
    found = re.search(r'\bdefault-lease-time\s+(\d+)', body)
    if found:
        pool['lease_time'] = found.group(1)
    return pool


def parse_dhcp_pools(content):
    """Parse every DHCP pool out of a dhcpd.conf.  Returns (pools, error).

    Every LLDPq installation written before multi-pool support nests its single
    subnet one level deeper, inside `shared-network OOB { ... }`.  That wrapper
    is a no-op only while it holds one subnet and carries no options of its
    own, and exactly that shape is unwrapped here.  Any other wrapper may carry
    real same-L2 semantics, so it is reported instead of guessed at: silently
    flattening it would hand out leases from the wrong subnet.
    """
    if not content or not content.strip():
        return [], ''
    scan, plain = _mask_dhcp_comments(content)
    if scan.count('{') != scan.count('}'):
        return [], (
            'This dhcpd.conf has unbalanced braces, so its pools cannot be '
            'read. Fix /etc/dhcp/dhcpd.conf by hand before saving from the '
            'Provision UI.'
        )
    pools = []
    for span_start, header, body_start, body_end in _iter_dhcp_blocks(
        scan, 0, len(scan)
    ):
        keyword = _dhcp_block_keyword(header)
        if keyword == 'subnet6':
            return [], (
                'This dhcpd.conf declares an IPv6 subnet6 block, which the '
                'Provision UI cannot represent. Edit /etc/dhcp/dhcpd.conf by '
                'hand or remove the subnet6 declaration before saving here.'
            )
        if keyword == 'subnet':
            pool = _read_dhcp_subnet_block(
                plain, header, body_start, body_end
            )
            if pool is None:
                return [], (
                    'A subnet declaration in dhcpd.conf could not be read: '
                    f'"{" ".join(header.split())}". Expected '
                    '"subnet <network> netmask <mask> {". Fix it by hand '
                    'before saving from the Provision UI.'
                )
            pool['name'] = _read_dhcp_pool_name_marker(
                content, span_start, body_start
            )
            pools.append(pool)
            continue
        if keyword != 'shared-network':
            continue

        nested = list(_iter_dhcp_blocks(scan, body_start, body_end))
        subnets = [item for item in nested
                   if _dhcp_block_keyword(item[1]) == 'subnet']
        wrapper_name = ' '.join(header.split())
        if len(subnets) != 1 or len(nested) != len(subnets):
            return [], (
                f'This dhcpd.conf has a "{wrapper_name}" block holding '
                f'{len(subnets)} subnet declaration(s). A shared-network means '
                '"these subnets are on the same wire", which LLDPq cannot '
                'express as separate pools. Review /etc/dhcp/dhcpd.conf by '
                'hand and keep one subnet per pool before saving here.'
            )
        subnet_start, subnet_header, subnet_body_start, subnet_body_end = subnets[0]
        outside = (scan[body_start:subnet_start]
                   + scan[subnet_body_end + 1:body_end])
        if outside.strip():
            return [], (
                f'This dhcpd.conf has a "{wrapper_name}" block that sets '
                'options at shared-network scope. Those options apply to every '
                'subnet on that wire and would change meaning if the wrapper '
                'were removed. Review /etc/dhcp/dhcpd.conf by hand before '
                'saving from the Provision UI.'
            )
        pool = _read_dhcp_subnet_block(
            plain, subnet_header, subnet_body_start, subnet_body_end
        )
        if pool is None:
            return [], (
                'A subnet declaration in dhcpd.conf could not be read: '
                f'"{" ".join(subnet_header.split())}". Fix it by hand before '
                'saving from the Provision UI.'
            )
        pool['name'] = (
            _read_dhcp_pool_name_marker(content, subnet_start, subnet_body_start)
            or _read_dhcp_pool_name_marker(content, span_start, body_start)
        )
        pools.append(pool)

    for index, pool in enumerate(pools):
        if not pool['name']:
            pool['name'] = f'Pool-{index + 1}'
    return pools, ''


def read_dhcp_conf_text():
    conf_path = os.environ.get('DHCP_CONF_FILE', '/etc/dhcp/dhcpd.conf')
    if not os.path.exists(conf_path):
        return ''
    try:
...
def read_dhcp_pools_from_live_config():
    return parse_dhcp_pools(read_dhcp_conf_text())


def dhcp_pools_from_parsed(parsed):
    """Convert parsed pools into the shape the shared validators compare.

    A hand-edited file may be missing a gateway or a range, so every field is
    optional here; a subnet that cannot be read at all is left out rather than
    turned into a verdict about someone's reservations.
    """
    pools = []
    for index, entry in enumerate(parsed):
        netmask = str(entry.get('netmask', '')).strip() or '255.255.255.0'
        try:
            network = ipaddress.IPv4Network(
                f'{str(entry.get("subnet", "")).strip()}/{netmask}', strict=False
            )
        except (ipaddress.AddressValueError, ipaddress.NetmaskValueError,
                ValueError):
            continue
        try:
            gateway = ipaddress.IPv4Address(str(entry.get('gateway', '')).strip())
        except ipaddress.AddressValueError:
            gateway = None
        try:
            range_start = ipaddress.IPv4Address(
                str(entry.get('range_start', '')).strip()
            )
            range_end = ipaddress.IPv4Address(
                str(entry.get('range_end', '')).strip()
            )
        except ipaddress.AddressValueError:
            range_start = range_end = None
        pools.append({
            'name': entry.get('name') or f'Pool-{index + 1}',
            'label': f'Pool {index + 1}',
            'network': network,
            'gateway': gateway,
...
def read_live_dhcp_pools():
    parsed, error = read_dhcp_pools_from_live_config()
    if error:
        return [], error
    return dhcp_pools_from_parsed(parsed), ''


def annotate_bindings_with_pool(bindings):
    """Label each reservation with the pool whose subnet contains its address."""
    pools, _ = read_dhcp_pools_from_live_config()
    networks = []
    for pool in pools:
        try:
            networks.append((
                ipaddress.IPv4Network(f'{pool["subnet"]}/{pool["netmask"]}',
                                      strict=False),
                pool['name'],
            ))
        except (ipaddress.AddressValueError, ipaddress.NetmaskValueError,
    ...
    for binding in bindings:
        binding['pool'] = ''
        try:
            address = ipaddress.IPv4Address(str(binding.get('ip', '')).strip())
        except ipaddress.AddressValueError:
            continue
...
def read_isc_dhcp_interface():
    """Read the configured listen interface, INTERFACESv4 first.

    INTERFACESv4 is the key Ubuntu/Debian's isc-dhcp-server actually honours;
    the bare INTERFACES fallback keeps installations written before that fix
    readable until their next save.
    """
    try:
        with open(ISC_DHCP_DEFAULT_FILE, 'r') as handle:
            content = handle.read()
    except OSError:
        return ''
    for key in ('INTERFACESv4', 'INTERFACES'):
        match = re.search(
            r'(?m)^\s*' + key + r'\s*=\s*"([^"]*)"\s*$', content
        )
...
def render_isc_dhcp_defaults(iface):
    return f'INTERFACESv4="{iface}"\nINTERFACESv6=""\n'


def interface_ipv4_addresses(iface):
    """Return (addresses, enumerated) for one interface.

    `enumerated` is False only when the address list could not be read at all,
    so a host without an `ip` binary is not told its pool is misconfigured.
    """
    try:
        result = subprocess.run(['ip', '-4', '-o', 'addr', 'show', 'dev', iface],
                                capture_output=True, text=True, timeout=5)
    ...
    for line in result.stdout.splitlines():
        ...
        if len(fields) < 4:
            continue
        try:
            addresses.append(ipaddress.IPv4Interface(fields[3]).ip)
        except (ipaddress.AddressValueError, ValueError):
            continue
    return addresses, True


def sanitize_dhcp_pool_name(value, fallback):
    """Validate a pool name for the marker comment it is written into.

    The value lands in a `#LLDPQ pool name="..."` line, so an unescaped
    newline would let an operator inject arbitrary dhcpd directives under it.
    """
    name = str(value or '').strip()
    if not name:
        return fallback
    if len(name) > DHCP_POOL_NAME_MAX_LENGTH:
        raise ValueError(
            f'Pool name must be at most {DHCP_POOL_NAME_MAX_LENGTH} characters'
        )
    if not DHCP_POOL_NAME_CHARSET.fullmatch(name):
        raise ValueError(
            'Pool name may only contain letters, digits, spaces, "-", "_" '
            'and "."'
        )
    return name


def normalize_dhcp_pool(payload, index, runtime_server_ip=None):
    """Validate one submitted pool and resolve everything generation needs."""
    if not isinstance(payload, dict):
        raise ValueError(f'Pool {index + 1} is not a valid object')
    label = f'Pool {index + 1}'
    name = sanitize_dhcp_pool_name(payload.get('name'), f'Pool-{index + 1}')

    subnet_text = str(payload.get('subnet', '')).strip()
    netmask_text = str(payload.get('netmask', '255.255.255.0')).strip() \
        or '255.255.255.0'
    if not subnet_text:
        raise ValueError(f'{label} ("{name}") requires a subnet')
    network = ipaddress.IPv4Network(f'{subnet_text}/{netmask_text}', strict=False)
    if str(network.network_address) != subnet_text:
        raise ValueError(
            f'{label} ("{name}") subnet must be the network address '
            f'({network.network_address})'
        )
    gateway = ipaddress.IPv4Address(str(payload.get('gateway', '')).strip())
    if gateway not in network or gateway in (network.network_address,
                                             network.broadcast_address):
        raise ValueError(
            f'{label} ("{name}") gateway must be a usable address inside '
            f'{network}'
        )
    dns = ipaddress.IPv4Address(
        str(payload.get('dns', '')).strip() or str(gateway)
    )
    range_start_text = str(payload.get('range_start', '')).strip()
    range_end_text = str(payload.get('range_end', '')).strip()
    if bool(range_start_text) != bool(range_end_text):
        raise ValueError(
            f'{label} ("{name}") dynamic range start and end must be set together'
        )
    range_start = range_end = None
    if range_start_text:
        ...
        if range_start not in network or range_end not in network:
            raise ValueError(
                f'{label} ("{name}") dynamic range must stay inside {network}'
            )
        if range_start in (network.network_address, network.broadcast_address) or \
           range_end in (network.network_address, network.broadcast_address):
            raise ValueError(
                f'{label} ("{name}") dynamic range cannot use network or '
                'broadcast addresses'
            )
        if int(range_start) > int(range_end):
            raise ValueError(
                f'{label} ("{name}") dynamic range start must not be after '
                'range end'
            )
    lease_time = int(str(payload.get('lease_time', DEFAULT_DHCP_LEASE_TIME)).strip()
                     or DEFAULT_DHCP_LEASE_TIME)
    if lease_time < 60 or lease_time > 31536000:
        raise ValueError(
            f'{label} ("{name}") lease time must be between 60 and 31536000 '
            'seconds'
        )
    domain = str(payload.get('domain', '')).strip() or 'example.com'
    if len(domain) > 253 or not re.fullmatch(r'[A-Za-z0-9.-]+', domain):
        raise ValueError(f'{label} ("{name}") has an invalid DHCP domain name')

    # An empty URL is the switch that turns provisioning off for this pool, so
    # it is never filled in here: options 72, 114 and 239 are then all omitted.
    provision_url = str(payload.get('provision_url', '')).strip()
    server_ip = None
    server_base_url = ''
    if provision_url:
        ...
        if parsed_url.scheme not in ('http', 'https') or not parsed_url.hostname:
            raise ValueError(
                f'{label} ("{name}") provision URL must be an absolute HTTP or '
                'HTTPS URL'
            )
        try:
            ...
            try:
                ...
                raise ValueError(
                    f'{label} ("{name}") provision URL hostname does not '
                    f'resolve to IPv4: {parsed_url.hostname}'
                ) from exc
        if runtime_server_ip is not None and server_ip != runtime_server_ip:
            ...
            if str(runtime_server_ip) not in resolved_addresses:
                raise ValueError(
                    f'{label} ("{name}") provision URL must resolve to Docker '
                    f'PROVISION_SERVER_IP ({runtime_server_ip})'
                )
            server_ip = runtime_server_ip
        server_base_url = f'{parsed_url.scheme}://{parsed_url.netloc}/'

    return {
        'name': name, 'label': label, 'network': network, 'gateway': gateway,
        'dns': dns, 'range_start': range_start, 'range_end': range_end,
        'lease_time': lease_time, 'domain': domain,
        'provision_url': provision_url, 'server_ip': server_ip,
        'server_base_url': server_base_url,
    }


def normalize_dhcp_pools(payloads, runtime_server_ip=None):
    if not isinstance(payloads, list) or not payloads:
        raise ValueError('At least one DHCP pool is required')
    if len(payloads) > MAX_DHCP_POOLS:
        raise ValueError(f'At most {MAX_DHCP_POOLS} DHCP pools are supported')
    pools = [normalize_dhcp_pool(payload, index, runtime_server_ip)
             for index, payload in enumerate(payloads)]
    for first in range(len(pools)):
        for second in range(first + 1, len(pools)):
            if pools[first]['network'].overlaps(pools[second]['network']):
                raise ValueError(
                    f'{pools[first]["label"]} ("{pools[first]["name"]}") '
                    f'{pools[first]["network"]} overlaps '
                    f'{pools[second]["label"]} ("{pools[second]["name"]}") '
                    f'{pools[second]["network"]}'
                )
    return pools


def validate_primary_pool_is_directly_attached(pools, iface):
    """Pool 1 must own an address on the listen interface.

    `dhcpd -t` performs no network operation and a save made while the service
    is stopped never exercises interface binding, so nothing else in the save
    path would notice that Pool 1 stopped being local.
    """
    addresses, enumerated = interface_ipv4_addresses(iface)
    if not enumerated:
        return
    primary = pools[0]
    if not addresses:
        raise ValueError(
            f'Listen interface {iface} has no IPv4 address, so '
            f'{primary["label"]} ("{primary["name"]}") cannot be directly '
            'attached. Address the interface first, or select another one.'
        )
    if any(address in primary['network'] for address in addresses):
        return
    raise ValueError(
        f'{primary["label"]} ("{primary["name"]}") {primary["network"]} does '
        f'not contain any IPv4 address of the listen interface {iface} '
        f'({", ".join(str(address) for address in addresses)}). Pool 1 is the '
        'directly attached pool; add a relayed pool for subnets that reach '
        'LLDPq through a DHCP relay.'
    )


def _binding_is_dhcp_managed(binding):
    return bool(binding.get('dhcp', True))


def _binding_address(binding):
    try:
        return ipaddress.IPv4Address(str(binding.get('ip', '')).strip())
    except ipaddress.AddressValueError:
        return None


def _describe_bindings(entries):
    return ', '.join(
        f'{hostname or "(unnamed)"} ({address})' for hostname, address in entries
    )


def validate_bindings_against_pools(bindings, pools):
    """Check the final reservation set against the final pool set.

    Returns a list of actionable errors.  Records with DHCP disabled are exempt:
    monitored supplements are legitimately outside every pool.  An address that
    lands in no pool, or inside a pool's dynamic range, is not a syntax error —
    dhcpd accepts it and then never serves it.
    """
    if not pools:
        return []
    orphans = []
    conflicts = []
    for binding in bindings:
        if not _binding_is_dhcp_managed(binding):
            continue
        address = _binding_address(binding)
        if address is None:
            continue
        hostname = str(binding.get('hostname', '')).strip()
        owners = [pool for pool in pools if address in pool['network']]
        if len(owners) != 1:
            orphans.append((hostname, address))
            continue
        pool = owners[0]
        reason = ''
        if address == pool['network'].network_address:
            reason = 'is the network address of'
        elif address == pool['network'].broadcast_address:
            reason = 'is the broadcast address of'
        elif pool['gateway'] is not None and address == pool['gateway']:
            reason = 'is the gateway of'
        elif pool['range_start'] is not None and \
                int(pool['range_start']) <= int(address) <= int(pool['range_end']):
            reason = 'is inside the dynamic range of'
        if reason:
            conflicts.append(
                f'{hostname or "(unnamed)"} ({address}) {reason} '
                f'{pool["label"]} ("{pool["name"]}")'
            )

    errors = []
    if orphans:
        errors.append(
            'These DHCP reservations are outside every configured pool: '
            f'{_describe_bindings(orphans)}. Add a pool that covers them, '
            'change their addresses, or turn DHCP off for them in Inventory.'
        )
    if conflicts:
        errors.append(
            'These DHCP reservations collide with a pool address: '
            + '; '.join(conflicts)
            + '. Change the address or the pool range.'
        )
    return errors


def validate_removed_pools(live_pools, pools, bindings):
    """Refuse to drop a pool that still serves reservations."""
    kept = {pool['network'] for pool in pools}
    errors = []
    for live in live_pools:
        ...
        for binding in bindings:
            ...
            if address is not None and address in network:
                affected.append((str(binding.get('hostname', '')).strip(), address))
        if affected:
            errors.append(
                f'Pool "{live["name"]}" ({network}) cannot be removed while '
                f'these DHCP reservations use it: {_describe_bindings(affected)}. '
                'Remove them from Inventory or move them into a pool you keep.'
            )
    return errors


def validate_pool_protected_addresses(pools, hosts_content):
    """Reject a dynamic range that covers an address dhcpd must not hand out.

    `dhcpd -t` accepts a reservation inside a dynamic pool, so the range could
    otherwise assign the gateway, the provisioning server or another device's
    fixed address to an unrelated client.  Each check is scoped to the pool the
    address belongs to; a reservation in one pool must not block another's range.
    """
    reservations = []
    for address in re.findall(r'\bfixed-address\s+([0-9.]+)\s*;', hosts_content):
        try:
            reservations.append(ipaddress.IPv4Address(address))
        except ipaddress.AddressValueError:
            continue
    for pool in pools:
        if pool['range_start'] is None:
            continue
        pool_start, pool_end = int(pool['range_start']), int(pool['range_end'])
        protected = {str(pool['gateway']): 'gateway'}
        if pool['server_ip'] is not None and pool['server_ip'] in pool['network']:
            protected[str(pool['server_ip'])] = 'provision server'
        for reserved in reservations:
            if reserved in pool['network']:
                protected[str(reserved)] = 'fixed reservation'
        overlaps = [
            f'{address} ({role})' for address, role in protected.items()
            if pool_start <= int(ipaddress.IPv4Address(address)) <= pool_end
        ]
        if overlaps:
            raise ValueError(
                f'{pool["label"]} ("{pool["name"]}") dynamic range overlaps '
                'protected address(es): ' + ', '.join(overlaps)
            )


DHCP_CONF_PREAMBLE = """# /etc/dhcp/dhcpd.conf - Generated by LLDPq
...
def render_dhcp_conf(pools, hosts_path):
    """Render dhcpd.conf with one sibling top-level subnet block per pool.

    The blocks are deliberately not wrapped in a `shared-network`: that would
    tell dhcpd the subnets share a wire and let it answer a relayed request
    from the wrong pool.
    """
    blocks = []
    for pool in pools:
        lines = [f'{DHCP_POOL_NAME_MARKER}"{pool["name"]}"',
                 f'subnet {pool["network"].network_address} netmask '
                 f'{pool["network"].netmask} {{']
        if pool['range_start'] is not None:
            lines.append(f'  range {pool["range_start"]} {pool["range_end"]};')
        else:
            lines.append('  # range not configured')
        lines.append(f'  option routers {pool["gateway"]};')
        lines.append(f'  option domain-name "{pool["domain"]}";')
        lines.append(f'  option domain-name-servers {pool["dns"]};')
        if pool['provision_url']:
            # Options 72 and 114 drive ONIE's network install discovery, so a
            # pool that does not provision must not advertise them either.
            lines.append(f'  option www-server {pool["server_ip"]};')
            lines.append(f'  option default-url "{pool["server_base_url"]}";')
            lines.append(
                f'  option cumulus-provision-url "{pool["provision_url"]}";'
            )
        lines.append(f'  default-lease-time {pool["lease_time"]};')
        lines.append(f'  max-lease-time     {pool["lease_time"] * 2};')
        lines.append('}')
        blocks.append('\n'.join(lines) + '\n')

    sections = [DHCP_CONF_PREAMBLE] + blocks + [f'include "{hosts_path}";\n']
    return '\n'.join(sections)


def action_get_dhcp_config():
    """Read dhcpd.conf and isc-dhcp-server defaults, return parsed settings."""
    pools, config_error = parse_dhcp_pools(read_dhcp_conf_text())

    # The flat keys mirror pool 1 so a page loaded before multi-pool support
    # still shows the primary pool instead of nothing.
    primary = pools[0] if pools else {}
    config = {
        ...
        'provision_url': primary.get('provision_url', ''),
        'lease_time': primary.get('lease_time', str(DEFAULT_DHCP_LEASE_TIME)),
        'interface': read_isc_dhcp_interface() or 'eth0',
    }

    # List network interfaces with every IPv4 address they carry: the relay
    # target shown on a relayed pool card is derived from the address that
    # falls inside pool 1, not from a generic first-match guess.
    interfaces = []
    try:
        ...
        if r.returncode == 0:
            ...
            for line in r.stdout.strip().split('\n'):
                ...
                if iface_name == 'lo':
                    continue
                entry = by_name.get(iface_name)
                if entry is None:
                    entry = {'name': iface_name, 'ip': ip_cidr.split('/')[0],
                             'ips': []}
                    by_name[iface_name] = entry
                    interfaces.append(entry)
                entry['ips'].append(ip_cidr)
    except Exception:
        pass
...
def _action_save_dhcp_config_locked():
    ...
    try:
        ...
        pool_payloads = data.get('pools')
        if pool_payloads is None:
            # A page cached from before multi-pool support submits the flat
            # single-pool shape.  Accepting it while the server has more pools
            # would silently delete them.
            live_pool_count = len(read_dhcp_pools_from_live_config()[0])
            if live_pool_count > 1:
                raise ValueError(
                    f'This page is out of date: the server has {live_pool_count} '
                    'DHCP pools but the request carries one. Reload Provision '
                    'and save again.'
                )
            pool_payloads = [{
                key: data.get(key, '') for key in (
                    'name', 'subnet', 'netmask', 'range_start', 'range_end',
                    'gateway', 'dns', 'domain', 'provision_url', 'lease_time',
                )
            }]
        pools = normalize_dhcp_pools(pool_payloads, runtime_server_ip)
        validate_primary_pool_is_directly_attached(pools, iface)
    ...
    hosts_path = get_dhcp_hosts_path()
    conf = render_dhcp_conf(pools, hosts_path)
    conf_path = os.environ.get('DHCP_CONF_FILE', '/etc/dhcp/dhcpd.conf')
    isc_default = ISC_DHCP_DEFAULT_FILE
    isc_content = render_isc_dhcp_defaults(iface)
    ...
    try:
        live_pools, live_pools_error = read_live_dhcp_pools()
        if live_pools_error:
            raise ValueError(live_pools_error)

        # Regenerate the reservations here too, so a file still carrying the
        # legacy `group { }` header converges on the flat shape whichever save
        # path runs first.
        bindings = dhcp_hosts_bindings_unlocked()
        hosts_content, _ = generate_dhcp_hosts(bindings)

        removal_errors = validate_removed_pools(live_pools, pools, bindings)
        if removal_errors:
            raise ValueError(' '.join(removal_errors))
        binding_errors = validate_bindings_against_pools(bindings, pools)
        if binding_errors:
            raise ValueError(' '.join(binding_errors))
        validate_pool_protected_addresses(pools, hosts_content)
    ...
    result_json({
        "success": True,
        "message": f"{len(pools)} pool(s) saved. DHCP: {msg}",
        "dhcp_restart": bool(was_running),
        "validated": True,
        "rollback_performed": rollback_performed,
        "pools": len(pools),
        "server_ip": (str(pools[0]['server_ip'])
                      if pools[0]['server_ip'] is not None else ''),
    })
...
if ACTION == 'discovery-worker':
    ...
    annotate_bindings_with_pool(bindings)
